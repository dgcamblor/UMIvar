...
Increasingly sized NGS panels have been developed for better tumor tissue characterization, 
...
In the same line, other commercial assays have also leveraged WES for tumor tissue characterization and variant targeting in ctDNA. In a preliminar report by Cao et al., they used brPROPHET to track up to 55 variants in the ctDNA of a cohort of 117 patients spanning stages I-IV. Although the conclusions on RFS and capability of predicting recurrence could not be draw due to the limited observation period, a direct comparison of the method with a fixed panel of 168 genes in the preoperative setting was provided. While brPROPHET detected ctDNA in 97.3% of the patients, the tumor-agnostic approach did so in 68.9%, highlighting the increased sensitivity of the tumor-informed approach. Further results that show the capacity of this assay remain to be seen [@caoPatientspecificTumorinformedCirculating2023]. Likewise, AlphaLiquid Detect follows a similar approach, and has been shown to provide a sensitivity of 61.9% and a specificity of 83.9% at a postoperative timepoint of 3 weeks for recurrence prediction up to 3 years after surgery [@ryooPersonalisedCirculatingTumour2023]. Further research is needed to validate these assays in the clinical setting.

#### WES in ctDNA characterization

On the ctDNA end of tumor characterization, the use of WES is also beginning to be explored in the context of MRD detection, recognizing that the greater genomic breadth analyzed would be useful in addressing the limitations of current tumor-agnostic panels. The use of targeted NGS panels is, however, still the preferential method for the detection of ctDNA variants due to their low frequency in plasma; targeted panels, albeit limited in the breadth of the genomic regions they cover, are able to achieve a higher depth of coverage at an adequate cost. Although the use of WES in ctDNA detection has proven feasible, its sensitivity and agreement with tumor tissue is highly dependent on the tumor fraction [@leenanitikulConcordanceWholeExome2023; @bosWholeExomeSequencing2020]. Lower sequencing costs, the implementation of noise correction technologies (e.g. UMIs) and bioinformatics developments will certainly facilitate the implementation of WES in the tumor-agnostic practice.

#### WGS

Likewise, whole-genome sequencing of cfDNA has great potential implications for the agnostic detection of MRD. The most prominent use case for genetic alterations has been the MRDetect algorithm, which calls ctDNA based on the detection of single nucleotide variants and copy number alterations in WGS [@zviranGenomewideCellfreeDNA2020]. A validation study was performed in a cohort from 146 CRC patients stage III, revealing the prediction of recurrence in 100% (5/5) patients, while maintaining a low false positive rate of 1% (1/100) [@frydendahlAbstract1959Sensitive2022].

### 2. Epigenomics and beyond

> Incorporation of other molecular markers cfRNA, miRNA...

Although genetic variants are by far the most assessed marker for detecting ctDNA/MRD, an increasing part of the attention has shifted to exploring the epigenomic landscape of ctDNA and, in particular, methylation. Indeed, aberrant methylation is a major player in tumorigenesis that, in contrast to tumor mutations, appears to be more consistent across tumors from different patients and stages [@garrigouStudyHypermethylatedCirculating2016; @haoDNAMethylationMarkers2017; @luoCirculatingTumorDNA2020]. As it can be inferred, the study of the methylome is promising for the agnostic assessment of MRD, in which sensitivity of detection is often hindered by the heterogeneity of the tumor mutational landscape. Of note, the clinical benefit of a tumor-informed methylation approach has been assesed in the MEDAL study, although a similar methodology remains to be seen in CRC [@chenIndividualizedDynamicMethylationbased2023].

Following the compelling evidence of the utility of methylation markers in the detection of ctDNA, the commercial assay Guardant Reveal (Guardant Health) has been developed to integrate the study of methylation and fragmentomics with the detection of genetic variants. Although the specific targets of the panel have not been disclosed, it spans approximately 500 kb of regions that are relevant to cancer both in terms of genetic and epigenetic alterations. The signal is interpreted by proprietary software, which emits the decision for ctDNA calling. In favor of the use of methylation markers, Parikh et al. reported that the addition of these signatures improved the sensitivity in detecting recurrence by 25-36% [@parikhMinimalResidualDisease2021]. The last iteration of the Guardant Reveal assay, called Guardant Infinity, aims at increasing the breadth of the panel to more than 700 genes and approximately 15 Mb of methylation-relevant regions. Although interim results of the COSMOS study seem to hint at the clinical utility of the assay, final results will be needed to confirm the benefit of the method [@nakamuraMultiomicAnalysisMinimal2024].
...
As more omic data become increasingly available and accessible, the integration of multiple omics is certainly a target to pursue for the promise of a more sensitive and specific detection of ctDNA. The integration between genomics, epigenomics, fragmentomics and proteomics (in this context, the identification of tumor-specific proteins) was recently pursued in a study for the early detection of CRC across all the stages, using the LUNAR-2 panel (Guardant Health). The sensitivity and the specificity reached values as high as 93% and 90%, thus surpassing the values of many of the gene-specific methylation assays. Notably, the sensitivity in stage I remained high (84%) [@bessaHighAccuracyBlood2023]. Although the assay is currently being tested for the early detection of CRC in the multi-center prospective trial ECLIPSE (NCT04136002), it would be interesting to explore the utility of the method in the MRD setting.

### 3. Potential new biomarkers in liquid biopsy

Other liquid biopsy analytes beyond ctDNA may have potential for addressing MRD, mainly in a tumor-agnostic manner.

Circulating cell-free RNA (cfRNA) and circulating tumor RNA (ctRNA) have been extensively studied as a liquid biopsy biomarkers. The analysis of cell-free mRNA shows valuable insights into the tumor transcriptome which has value for the tumor detection and characterization. Different studies have presented that the expression of genes such as SOX9, RHOA, G2K3A, HPGD, PACS1, TDP2 hold promise for the detection and prognosis of CRC [@jinCellfreeCirculatingTumor2023; @xueNoninvasivePotentialCirculating2019; @xueNoninvasiveDiagnosisColorectal2023]. MicroRNAs (miRNAs) are small non-coding RNA molecules that regulate gene expression and have been shown to be dysregulated in cancer. Studies have proposed  miRNA signatures correlated with the presence of disease and higher risk of recurrence, pointing to specific miRNAs such as miR-21 [@wangIdentificationCirculatingMicroRNA2014; @moratalla-navarroIdentificationTwelvemicroRNASignature2023; @menendezPrognosticImplicationsSerum2013]. The analysis of cfRNA is, nonetheless, difficult due to the abundance of RNases in blood, which degrade the RNA molecules and make their detection challenging. The use of RNA-stabilizing agents and the development of new technologies for the detection of cfRNA are currently being explored to address this limitation.

Circulating tumor cells are also a potential MRD biomarker that has been explored in the tumor-agnostic setting. These are cells that have shed from the tumor and migrate into the peripheral blood circulation. Research has shown that the detection of CTCs in the postoperative setting is correlated with worse RFS in CRC [@rahbariMetaanalysisShowsThat2010]. Indeed, a prospective study in 130 CRC patients stage II-III hinted at the clinical utility of their use as an independent marker for predicting recurrence (HR = 2.739, 95% CI=1.042 - 7.200, p = 0.041) [@wangPrognosticModelsBased2019]. Their isolation is, however, problematic due to a lack of molecular markers.

The study of all of these novel tumor-agnostic liquid biopsy biomarkers is currently hindered by the absence of standardized methods to assess them, leading to limitations in the sensitivity and specificity for their detection. To acknowledge their potential, future research addressing their shortcomings is needed.
